/* Author: Winchell Chung AKA Nyrath
 * License: This work is licensed under a Creative Commons Attribution-ShareAlike 3.0 Unported 3.0 Unported License. 
...
{
	// Constants
	public enum MoveState {Motionless, BombFlight, NeutralToCompress, CompressToNeutral, NeutralToExtend, ExtendToNeutral};
	...
	public class OrionPusherPlate : Part
	{
		...
		[KSPField]
	    public float plateOffsetCompressed = 4.88f;	// distance pusher plate moves upward above neutral point in animation
		[KSPField]
	    public float plateOffsetExtended = 0.4f;	// distance pusher plate moves downward below neutral point when recoiling 
		[KSPField]
		...
		[KSPField]
		public float plateBottomY = -12.5655f;		// distance between vessel center and bottom of pusher plate, negative number
		[KSPField]
		public float bombStartY = -5.0f;			// distance between vessel center and start postion of bomb, negative number
		[KSPField]
		public float detonationDeltaY = -23f;		// distance between bottom of pusher plate and detonation point in meters, negative number
		[KSPField]
		public string engineModelName = "USAF10MeterOrionEngine2";	// name of the Unity 3D model
		[KSPField]
		public string subModelPlateName = "obj_pusherplate";		// name of the pusher plate sub model in the Unity 3D model
		[KSPField]
		...
		public float detonationY = 0f;				// distance between vessel center and detonation point, = plateBottomY + detonationDeltaY
		...
		private Dictionary<string, string> bombFilterActivateEvents = new Dictionary<string, string>();	// list of activate events for bomb filter pop-ups
		private Dictionary<string, string> bombFilterDeactivateEvents = new Dictionary<string, string>();	// list of deactivate events for bomb filter pop-ups
		private MoveState pusherMoveState = MoveState.Motionless;	// what part of a pusher cycle the animiation is currently in	
		...
		private float plateCycleSpeed;				// how fast plate moves in meters per second, used to time the segments of pusher plate animation
		...
		private float nextFire = 0f;				// next allowed bomb detonatation time, used to delay bombs launched by Z key
		private int lastNumBombs = 0;				// cache, only update number of bomb indicator if the number has actually changed
		private float lastThrottle = 0f;			// cache, only print throttle change msg if throttle has changed
		public List<String> magazineKeys  = new List<String>();
		private Dictionary<String, String> magazineTitles = new Dictionary<String, String>();
		private int maxMagazine = 0;
		private int maxEventSlots = 10;				// must match number of activate/deactivate KSPEvents
		private int maxActionSlots = 10;			// must match number of activate/deactivate KSPActions
		private List<Transform> pushAnimationTransforms = new List<Transform>();	// parts attached to pusher plate, are animated with the plate
		private Dictionary<float, float> sineRate = new Dictionary<float, float>()	// table for incrementing with sine-wave spacing
		{
		    { 0.1f, 1.56f},
		    { 0.2f, 1.53f},
		    { 0.3f, 1.45f},
...
//		private Stack<int> dockedShipIDStack = new Stack<int>();
		private float bombTravelDistance = 0f;
//		private LineRenderer debugLine = null;
		private float newBombYStart = 0f;
		private bool bombObstacle = false;
		...
		protected override void onPartAwake()
		{
			...
			foreach (AvailablePart eachPart in PartLoader.LoadedPartsList) {
				Part thePart = eachPart.partPrefab;
				if (thePart != null) {
					if (thePart.ClassName == "OrionMagazine") {
						OrionMagazine theMagazine = thePart as OrionMagazine;
			...
			// deactive them all
			foreach (BaseAction theAction in Actions) {
			...
			foreach (string magKey in this.magazineKeys) {
				...
				Actions[activateActionName].guiName = "Start " + this.magazineTitles[magKey];
				Actions[deactivateActionName].guiName = "Kill " + this.magazineTitles[magKey];
	            Actions[activateActionName].active = true;
	            Actions[deactivateActionName].active = true;
	    ...
	    protected override void onFlightStart()
	    {
			...
			this.newBombYStart = this.bombModel.localPosition.y;	// to reset bomb to original position at end of animation
			this.bombTravelDistance = Math.Abs((this.plateBottomY + this.detonationDeltaY) - this.bombModel.localPosition.y);

			if (this.pusherPlate == null) {
				Debug.Log(">>> Debug.Log OrionPusherPlate:onFlightStart() ***ERROR: failure to retrieve pusher plate transform");
			}
			...
			float plateTopYNormalized = this.plateTopY + this.orgPos.y;
			foreach (Part aPart in this.children) {
...
//			flare_emitter.useWorldSpace = false;
			...
			foreach (string magKey in this.magazineKeys) {
				...
				Events[activateFunctionName].guiName = this.magazineTitles[magKey] + " off  (Start->)";
				Events[deactivateFunctionName].guiName = this.magazineTitles[magKey] + " feeding  (Kill->)";
				
	            Events[activateFunctionName].active = false;
	            Events[activateFunctionName].guiActive = false;
	            Events[deactivateFunctionName].active = true;
	            Events[deactivateFunctionName].guiActive = true;
...
//		}
	    ...
	    protected bool launchBomb()
	    {
	        if (this.vessel.isCommandable) {
				...
				if (this.aNukeRound.isBomb) {
					...
					this.bombObstacle = this.calcDetonationPoint();	// make sure bomb does not hit launch pad or anything
		        ...
		        } else {	// bomb launch fail, no nukes available in allowed magazines
					return false;
				}
			} else {	// bomb launch fail, vessel is not commandable
				return false;
			}
	    }
		...
		{
			RaycastHit hit;
			Vector3 bombPosition = new Vector3(0f, 0f, 0f);
			Vector3 bombDirection = new Vector3(0f, -1f, 0f);
			...
			Vector3d plateZero = new Vector3d(this.bombModel.localPosition.x, this.entireModel.localPosition.y+this.plateBottomY, this.bombModel.localPosition.z);
			...
			Vector3d groundZero = new Vector3d(this.bombModel.localPosition.x, this.entireModel.localPosition.y+this.plateBottomY+this.detonationY, this.bombModel.localPosition.z);
			...
			float bombMoveDistance = 0f;
			if (this.bombModel != null) {
				bombMoveDistance = Math.Abs(this.detonationY - this.bombStartY);
			}
			float totalPlateMoveDistance = (2 * this.plateOffsetCompressed) + (2 * this.plateOffsetExtended);
			float totalMovementDistance = bombMoveDistance + totalPlateMoveDistance;
			float movePercent = this.plateOffsetCompressed / totalMovementDistance;	// % NeutralToCompress travel is of total animation movement distance
			this.theDuration = movePercent * this.plateCycleTime; 					// duration of NeutralToCompress segment
			this.plateCycleSpeed = this.plateOffsetCompressed / this.theDuration;	// speed of plate, rate = distance / time
		...
		protected void detonateBomb()
		{
            ...
            this.temperature += this.aNukeRound.bombHeat;
			...
			Vector3d groundZero = new Vector3d(this.bombModel.localPosition.x, this.entireModel.localPosition.y+this.detonationY, this.bombModel.localPosition.z);
			...
			GameObject nukeFlashHolder = new GameObject("flashHolder");
...
//			Debug.Log(">>> OrionPusherPlate.detonateBomb: plasma plume started at " + Time.time.ToString());
            ...
            // add velocity
            this.vessel.ChangeWorldVelocity(base.transform.up * (this.aNukeRound.bombImpulse / totalVesselMass));
            ...
            base.rigidbody.AddRelativeForce(new Vector3(0f, (this.aNukeRound.bombImpulse / totalVesselMass), 0f), ForceMode.Force);
			...
			foreach (Collider hit in colliders) {
				if (hit.attachedRigidbody != null) {
					Part hitPart = hit.collider.attachedRigidbody.GetComponent<Part>();
					// do not damage yourself, your ship is immune from your own nukes
					if (hitPart.vessel.id != this.pusherOwnerGuid) {
						hitPart.temperature += this.aNukeRound.bombHeat;		// add heat
						hitPart.vessel.GoOffRails();
						hit.attachedRigidbody.AddExplosionForce(this.aNukeRound.damageShock, groundZero, this.aNukeRound.damageZone, 0.0f);
					}
				}
			}
			...
			foreach (Collider hit in colliders) {
				if (hit.attachedRigidbody != null) {
					...
					// do not damage yourself, your ship is immune from your own nukes
					if (hitPart.vessel.id != this.pusherOwnerGuid) {
						if (hitPart.mass < this.aNukeRound.destroyMass) {	// spare any part over destroyMass tons, like asteroids
	    ...
	    protected override void onActiveFixedUpdate()
	    {
			...
			float mainThrottleSetting = this.vessel.ctrlState.mainThrottle;
			if (this.lastThrottle != mainThrottleSetting) {
				this.lastThrottle = mainThrottleSetting;
				if (mainThrottleSetting < 0.01f) {			// throttle at 0%
					this.detonationDelay = -1f;
				} else if (mainThrottleSetting < 0.3f) {	// throttle from 10% to 30%
					this.detonationDelay = this.detonationMaxDelay;
				} else if (mainThrottleSetting < 0.6f) {	// throttle from 30% to 60%
					this.detonationDelay = this.detonationStdDelay;
				} else {									// throttle form 60% to 100%
					this.detonationDelay = this.detonationMinDelay;
				}
			}
			
			// Automatic bomb launch
			if (this.detonationDelay > 0f) {			// throttle is non-zero
				if (this.canFire) {						// no explosion in progress
					if (Time.time > this.nextFire) {	// at least detonationDelay has elapsed since last bomb
...
//			}
			...
			if (this.pusherMoveState == MoveState.BombFlight) {
				...
				if (!this.bombObstacle) {
					...
					float newBombX = this.bombModel.localPosition.x;
					float newBombY = this.bombModel.localPosition.y;
					float newBombZ = this.bombModel.localPosition.z;
					
					// multiply by Time.get_deltaTime to convert plateCycleSpeed into displacement per second
					float bombDelta = this.plateCycleSpeed * Time.deltaTime;
				
					newBombY -= bombDelta;	// move down
				
					if (newBombY < this.bombOffset) {	// bomb has reached the finish point, MoveState.BombFlight is over
				...
				float quinterCyclePercent = (Time.time - this.quinterCycleStart) / this.theDuration;	// percentage of 1/5 cycle time consumed
				if ((this.pusherMoveState == MoveState.CompressToNeutral) || (this.pusherMoveState == MoveState.ExtendToNeutral)) {
				...
				if (quinterCyclePercent < 0.1f) {		// keep in bounds of key values
					quinterCyclePercent = 0.1f;
				} else if (quinterCyclePercent > 1f) {
					quinterCyclePercent = 1f;
				}
				float sineFactor = this.sineRate[quinterCyclePercent];
				
				// multiply by Time.get_deltaTime to convert plateCycleSpeed into displacement per second
				float plateDelta = (this.plateCycleSpeed * sineFactor) * Time.deltaTime;	// scale plateCycleSpeed by sine wave
				...
				foreach (Transform aniTransform in this.pushAnimationTransforms) {
	    ...
	    protected override void onPartFixedUpdate()
	    {
	        ...
	        if (this.bombCounter == null) {
				...
				int numBombs = nukeManager.effectiveStockpileSize();
...
//					this.stackIcon.SetIconColor(Color.red); TODO
			...
			foreach(KeyValuePair<string, bool> hasMagazineEntry in this.nukeManager.hasMagazine) {
				...
				if (hasMagazineValue) {
					if ((Events[activateFunctionName].active == false) && (Events[deactivateFunctionName].active == false)) {
			            Events[activateFunctionName].active = false;
						Events[activateFunctionName].guiActive = false;
			            Events[deactivateFunctionName].active = true;
			            Events[deactivateFunctionName].guiActive = true;
		            ...
		            Events[activateFunctionName].active = false;
					Events[activateFunctionName].guiActive = false;
		            Events[deactivateFunctionName].active = false;
		            Events[deactivateFunctionName].guiActive = false;
				}
			}
		...
		private void activateBombEvent(string activateFunctionName, string deactivateFunctionName, string filterKey)
		{
			...
			this.nukeManager.unfilterMagazine(filterKey);	// if no magazine of this type attached, nukeManager knows to do nothing
		}

		private void deactivateBombEvent(string activateFunctionName, string deactivateFunctionName, string filterKey)
		{
            Events[activateFunctionName].active = true;
            Events[activateFunctionName].guiActive = true;
            Events[deactivateFunctionName].active = false;
            Events[deactivateFunctionName].guiActive = false;
			
			this.nukeManager.filterMagazine(filterKey);	// if no magazine of this type attached, nukeManager knows to do nothing
		}
